...
resource "aws_s3_bucket" "tf_state_bucket" {
  ...
  tags = merge(local.common_tags, map("name", "eks-tf-state-bucket-${var.environment}"))
}
...
resource "aws_dynamodb_table" "dynamodb-terraform-state-lock" {
  ...
  tags = merge(local.common_tags, map("name", "eks-tf-state-db-${var.environment}"))
}
...
resource "aws_s3_bucket" "s3_artifactory_bucket" {
  ...
  tags = merge(local.common_tags, map("name", "eks-aritifactory-bucket-${var.environment}"))
}
...
resource "aws_s3_bucket" "s3_dataLake_bucket" {
  ...
  tags = merge(local.common_tags, map("name", "eks-datalake-bucket-${var.environment}"))
}
